...
async function selectDayRange(page, daysToSelect) {
...
export default async function runDunning(browser, days, credentials) {
  ...
  await page2.goto(dunningPageURL, { waitUntil: 'domcontentloaded' });

  try {
    await selectDayRange(page2, days);
    ...
    const cred = await enterCredentials(page2, credentials);
    if (!cred) return;
